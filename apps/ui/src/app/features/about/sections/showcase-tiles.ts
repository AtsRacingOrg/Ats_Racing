import { ChangeDetectionStrategy, Component } from '@angular/core';
import { RevealDirective } from '../../../shared/directives/reveal.directive';
import { TranslatePipe } from '../../../core/i18n/translate.pipe';

interface Tile {
  readonly titleKey: string;
  readonly tagKey: string;
  readonly image: string;
}

@Component({
  selector: 'app-about-showcase',
  standalone: true,
  imports: [RevealDirective, TranslatePipe],
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `
    <section class="atiles" aria-label="Atölyemizden kareler">
      <ul role="list">
        @for (t of tiles; track t.titleKey; let i = $index) {
          <li class="atile" appReveal="fade-up" [revealDelay]="i * 130">
            <img [src]="t.image" [alt]="t.titleKey | t" loading="lazy" decoding="async" />
            <div class="atile__veil" aria-hidden="true"></div>
            <div class="atile__copy">
              <p class="atile__tag">{{ t.tagKey | t }}</p>
              <h3 class="atile__title">{{ t.titleKey | t }}</h3>
            </div>
          </li>
        }
      </ul>
    </section>
  `,
  styleUrl: './showcase-tiles.scss',
})
export class AboutShowcase {
  protected readonly tiles: readonly Tile[] = [
    { titleKey: 'about.showcase.1.title', tagKey: 'about.showcase.1.tag',
      image: 'https://images.unsplash.com/photo-1486006920555-c77dcf18193c?auto=format&fit=crop&w=1200&q=80' },
    { titleKey: 'about.showcase.2.title', tagKey: 'about.showcase.2.tag',
      image: 'https://images.unsplash.com/photo-1492144534655-ae79c964c9d7?auto=format&fit=crop&w=1200&q=80' },
    { titleKey: 'about.showcase.3.title', tagKey: 'about.showcase.3.tag',
      image: 'https://images.unsplash.com/photo-1605559424843-9e4c228bf1c2?auto=format&fit=crop&w=1200&q=80' },
  ];
}

-- =============================================================================
-- ATS Racing — Güvenlik sıkılaştırma
--  1) create_order: stage fiyatı yok/0 ise sipariş reddedilir (bedava sipariş engeli).
--     (UI'da "fiyat için iletişime geçin" olan seviyeler API'den de açılamaz.)
--  2) anon (kimliksiz) rolünden gereksiz fonksiyon EXECUTE yetkileri geri alınır.
-- =============================================================================
...
begin
  ...
  select role, country into v_role, v_country from profiles where id = v_uid;
  if v_role is null then raise exception 'profile not found'; end if;

  if v_country = 'TR' and not has_complete_billing(v_uid) then
    raise exception 'billing required' using errcode = 'P0001';
  end if;

  v_stage := (payload->>'stage')::tuning_stage;
  select price into v_base from tuning_prices where stage = v_stage;

  -- Fiyatı tanımlı olmayan / 0 olan seviye online sipariş edilemez (bedava sipariş engeli).
  if v_base is null or v_base <= 0 then
    raise exception 'price unavailable' using errcode = 'P0001';
  end if;
  ...
  if v_role = 'dealer' then
    ...
    insert into payments (user_id, order_id, amount, method, status, paid_at, provider_ref)
      values (v_uid, v_oid, v_base + v_extra, 'card', 'succeeded', now(), 'MOCK-' || v_oid);
  end if;
...
-- anon rolünden gereksiz fonksiyon yetkilerini geri al
revoke execute on function has_complete_billing(uuid) from anon, public;
revoke execute on function sync_email_confirmed() from anon, public;

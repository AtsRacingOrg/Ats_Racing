-- =============================================================================
-- ATS Racing — Kullanıcı ülkesi (profiles.country) + ülkeye bağlı fatura zorunluluğu
--
--  • profiles.country: ISO-3166-1 alpha-2 kodu (örn. 'TR', 'DE'). Varsayılan 'TR'.
--  • Mevcut kayıtlar 'TR' yapılır (önceki tüm müşteriler Türk kabul edilir).
--  • handle_new_user trigger'ı kayıt metadata'sından country okur (yoksa 'TR').
--  • Fatura bilgisi YALNIZCA Türkiye (country='TR') müşterileri için zorunlu.
-- =============================================================================

-- 1) Kolon + mevcut kayıtların backfill'i
alter table public.profiles
  add column if not exists country text not null default 'TR';

update public.profiles set country = 'TR' where country is null or country = '';

-- 2) Signup trigger — metadata'dan country (boşsa 'TR')
create or replace function public.handle_new_user()
  returns trigger language plpgsql security definer set search_path = public as $$
begin
  insert into public.profiles (id, email, role, full_name, phone, dealership_name, country, status)
  values (
    new.id,
    new.email,
    ...
    upper(coalesce(nullif(new.raw_user_meta_data->>'country', ''), 'TR')),
    'pending'
  )
...
-- 3) Sipariş oluşturma — fatura yalnızca Türk müşteriler için zorunlu

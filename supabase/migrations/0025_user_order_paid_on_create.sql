-- =============================================================================
-- ATS Racing — Normal müşteri siparişi peşin ödemeyle oluşur → ödeme 'succeeded'
--  • Bayi siparişleri ekstre/cari ile ilerler (dokunulmaz).
--  • Mevcut müşteri sipariş ödemeleri (iptal olmayan) succeeded yapılır.
-- =============================================================================
...
begin
  ...
  if v_role = 'dealer' then
    ...
    -- Normal müşteri: peşin ödeme alınmış kabul edilir.
...
-- Mevcut müşteri sipariş ödemeleri (iptal olmayan, ekstre dışı): pending → succeeded
update payments p
  set status = 'succeeded',
      paid_at = coalesce(p.paid_at, now()),
      provider_ref = coalesce(p.provider_ref, 'MOCK-' || p.order_id)
from orders o
where p.order_id = o.id
  and p.statement_id is null
